...
| `mutex({ id })` | `sync.mutex({ id })` | `Lock.value` → `Lock.ownerToken`; new monotonic `Lock.fence` (bigint) for fencing stale writers. `withLockOrThrow` is gone — check `withLock`'s `null`. TTLs round up to whole seconds. |
